...
ERROR_CRON_SCHEDULE=${ERROR_CRON_SCHEDULE:-"50 * * * *"}
...
if [ "$RUN_ONCE" = "false" ]; then
    ...
    echo "Setting tracker error cron schedule as ${ERROR_CRON_SCHEDULE}"
    ...
    echo "${ERROR_CRON_SCHEDULE} python /app/qbittorrent_jobs/tracker_error_tagger.py" >> $HOME/crontab
